...
x = ms.connect(host='localhost',user='root',passwd='--enter your password--',database='athish')
    ...
    if ch in [1,2,3,4,5,6,7]:
        if ch == 1:
            ...
            while boolean:
                ...
                cur.execute('select qty from product where id={}'.format(item_id))
                data2 = cur.fetchall()
                len = data2[0]
                if len == 0 :
                    ...
                    no_itemsadded = int(input('enter the qty of items to be added to the basket : '))
                    cur.execute('select price_per_item,name from product where id={}'.format(item_id))
                    data2 = cur.fetchone()
                    pu,cname = data2[0],data2[1]
                    cost = pu * no_itemsadded
                    cur.execute("insert into customer values('{}',{},{})".format(cname,no_itemsadded,cost))
                    print('\n item added succesfully😊')
                    x.commit()
                    cur.execute("update product set qty=qty-{} where id={}".format(no_itemsadded,item_id))
            ...
            display_basket()
            itemname = input('enter item name to remove the item from the basket : ')
            if check_item(itemname):
                print('entered item is present in the market,')
                ch4 = input('confirmation for removal of item from the basket : (ok/no) ')
                if ch4 == 'ok':
                    cur.execute("select sum(qty) from customer group by product_name having product_name = '{}' ".format(itemname))
                    data3 = cur.fetchall()

                    cur.execute("delete from customer where product_name = '{}'".format(itemname))
                    x.commit()
                    print('item removed succesfully')

                    cur.execute("update product set qty=qty+{} where name='{}'".format(data3[0][0],itemname))
                    x.commit()
                ...
                print('the entered item is not avaliable in the basket please enter valid name...')
                print('returning home....')
                continue
            ...
            cur.execute("insert into feedback values('{}','{}')".format(name,review))
            x.commit()
